...
<div class="preview" ref="preview" @mousedown="startDragRotate" @mousemove="dragRotateProduct" @mouseup="stopDragRotate" @click ="deselectProduct">
    
    <img src="../assets/lawn.jpg" alt="Uploaded Image" style="width: 100%; height: 100vh;">

    <img v-for="(product, index) in selectedProducts" :key="index" :src="product.image" :alt="product.name" class="product"
         :style="{ top: product.top + 'px', left: product.left + 'px', width: product.width + 'px', height: product.height + 'px',
        transform: 'translateZ(' + product.depth + 'px) rotateX(' + product.rotationX + 'deg) rotateY(' + product.rotationY + 'deg) rotateZ(' + product.rotationZ + 'deg) scale(' + product.scale + ')' }"
         ...
         <div v-if="selectedProductIndex !== null && !dragMoving && !dragResizing" class="circle horizontal-circle" :style="{ left: selectedProducts[selectedProductIndex].left + selectedProducts[selectedProductIndex].width / 2 + 'px', top: selectedProducts[selectedProductIndex].top + 'px' }"></div>
         <div v-if="selectedProductIndex !== null && !dragMoving && !dragResizing" class="circle vertical-circle" :style="{ left: selectedProducts[selectedProductIndex].left + 'px', top: selectedProducts[selectedProductIndex].top + selectedProducts[selectedProductIndex].height / 2 + 'px' }"></div>
